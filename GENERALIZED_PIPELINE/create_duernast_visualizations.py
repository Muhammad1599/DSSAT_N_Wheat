    ...
    try:
        ...
        for run_num in range(1, min(16, len(run_sections))):
            ...
            if data_rows:
                ...
                cumsum_wsgd = df['WFGD'].cumsum()
                df['cumulative_water_stress'] = cumsum_wsgd / (df.index + 1)  # Average daily stress
                df['daily_nitrogen_stress'] = df['NFTD']  # 1=optimal, 0=stressed
                cumsum_nstd = df['NSTD'].cumsum()
                df['cumulative_nitrogen_stress'] = cumsum_nstd / (df.index + 1)  # Average daily stress
...
def parse_observed_data(n_levels=None):
    ...
    try:
        ...
        if wht_file.exists():
            print(f"  [OK] Using {FILE_PREFIX}.WHT (detailed data: yield + grain weight + grain N)")
...
def create_comprehensive_visualization(treatments_data, phenology_stages, consensus_stages, 
    ...
    plot_configs = [
        ...
        {'idx': 1, 'var': 'cumulative_water_stress', 'title': 'b) Cumulative Water Stress (avg daily WSGD)', 'ylabel': 'Avg Daily WSGD\n(0=optimal, higher=more stress)'},
        {'idx': 2, 'var': 'cumulative_nitrogen_stress', 'title': 'c) Cumulative Nitrogen Stress (avg daily NSTD)', 'ylabel': 'Avg Daily NSTD\n(0=optimal, higher=more stress)'},
        ...
        {'idx': 6, 'var': 'H#AD', 'title': 'g) Grain Number', 'ylabel': 'Grains/m²'},
    ...
    for config in plot_configs:
            ...
            if var == 'grain_size_mg' and observed_data and consensus_stages:
                ...
                for trt_name in treatments_data.keys():
                    if trt_name in observed_data and 'grain_weight' in observed_data[trt_name]:
                        ...
                        # Only plot if we have valid observed grain weight data
                        if obs_grain_wt is not None and obs_grain_wt > 0:
        ...
        if var not in ['yield_comparison']:
            ...
            # Add date labels to weather panel x-axis (after ticks are set)
            if var == 'weather' and phenology_stages and len(phenology_stages) > 0:
                first_stage = list(phenology_stages.values())[0]
        ...
        if var not in ['yield_comparison']:
            if var == 'weather':
                ax.set_xlabel('Days After Planting (DAS) / Date', fontsize=9)
            else:
